...
## Gun carrying over the life course {background-image="img/stephan-cassara-KnAIsBuitGg-unsplash.jpg" background-opacity="0.3"}
...
* Key measures:
   * **Initiation**: Age first carried a concealed gun
...
## [Carry and onset of gun use]{.r-fit-text}{background-image="img/pedro-lastra-5Bv3EcijAl4-unsplash.jpg" background-opacity="0.1"}
...
* 74% of young carriers used gun by 21
    * Those who didn't never used a gun after 21
...
continuity_plot_data |> 
  mutate(Initiation = factor(ifelse(pre_21, "Before\n21", "After\n21"), levels = c("Before\n21", "After\n21")),
  ...
  geom_col(position = "dodge") +
   geom_text(data = ~ . |> distinct(Initiation) |>
               mutate(pr = 0.07,
                      x = ifelse(Initiation == "Before\n21", .77, 1.23)),
            aes(x = x, label = Initiation, y = pr), color = "black", vjust = 0.5, hjust = 0.5, size = 9, lineheight = .8,
            family = plot_font) +
  theme_minimal(base_size = 24) +
